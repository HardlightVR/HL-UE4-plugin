...
static FCompiledInDeferEnum Z_CompiledInDeferEnum_UEnum_HapticFileType(HapticFileType_StaticEnum, TEXT("/Script/HapticSuit"), TEXT("HapticFileType"), false, nullptr, nullptr);
	...
	IMPLEMENT_CLASS(UHapticBlueprintLibrary, 856315186);
...
// Cross Module References
	...
	UEnum* Z_Construct_UEnum_HapticSuit_HapticFileType()
	{
		...
		if (!ReturnEnum)
		{
			...
			EnumNames.Add(TPairInitializer<FName, uint8>(FName(TEXT("HapticFileType::Sequence")), 0));
			EnumNames.Add(TPairInitializer<FName, uint8>(FName(TEXT("HapticFileType::SequenceOnAll")), 1));
			EnumNames.Add(TPairInitializer<FName, uint8>(FName(TEXT("HapticFileType::Pattern")), 2));
			EnumNames.Add(TPairInitializer<FName, uint8>(FName(TEXT("HapticFileType::Experience")), 3));
			EnumNames.Add(TPairInitializer<FName, uint8>(FName(TEXT("HapticFileType::HapticFileType_MAX")), 4));
	...
	uint32 Get_Z_Construct_UEnum_HapticSuit_HapticFileType_CRC() { return 1733858194U; }
	...
	UFunction* Z_Construct_UFunction_UHapticBlueprintLibrary_PlayHaptic()
	{
		...
		if (!ReturnFunction)
		{
			...
			UProperty* NewProp_fileType = new(EC_InternalUseOnlyConstructor, ReturnFunction, TEXT("fileType"), RF_Public|RF_Transient|RF_MarkAsNative) UByteProperty(CPP_PROPERTY_BASE(fileType, HapticBlueprintLibrary_eventPlayHaptic_Parms), 0x0010000000000080, Z_Construct_UEnum_HapticSuit_HapticFileType());
	...
	UClass* Z_Construct_UClass_UHapticBlueprintLibrary()
	{
		...
		if (!OuterClass)
		{
			...
			if (!(OuterClass->ClassFlags & CLASS_Constructed))
			{
				...
				OuterClass->AddFunctionToFunctionMapWithOverriddenName(Z_Construct_UFunction_UHapticBlueprintLibrary_PlayHaptic(), "PlayHaptic"); // 655601339
